...
describe('OrderSummary component', () => {
  ...
  it('renders cart items correctly', () => {
    ...
    expect(within(cartItemContainers[0]).getByTestId('cart-item-image')).toHaveAttribute(
      'src',
      'images/products/athletic-cotton-socks-6-pairs.jpg'
    );
    expect(within(cartItemContainers[0]).getByTestId('cart-item-name')).toHaveTextContent(
      'Black and Gray Athletic Cotton Socks - 6 Pairs'
    );
    expect(within(cartItemContainers[0]).getByTestId('cart-item-price')).toHaveTextContent(
      '$10.90'
    );
    expect(within(cartItemContainers[0]).getByTestId('cart-item-quantity')).toHaveTextContent(
      'Quantity: 2'
    );
    ...
    expect(within(cartItemContainers[1]).getByTestId('cart-item-image')).toHaveAttribute(
      'src',
      'images/products/intermediate-composite-basketball.jpg'
    );
    expect(within(cartItemContainers[1]).getByTestId('cart-item-name')).toHaveTextContent(
      'Intermediate Size Basketball'
    );
    expect(within(cartItemContainers[1]).getByTestId('cart-item-price')).toHaveTextContent(
      '$20.95'
    );
    expect(within(cartItemContainers[1]).getByTestId('cart-item-quantity')).toHaveTextContent(
      'Quantity: 1'
    );

    deliveryOptionInputs = within(cartItemContainers[1]).getAllByTestId('delivery-option-input');
    expect(deliveryOptionInputs.length).toBe(3);
  ...
  it('deletes cart item', async () => {
    ...
    await user.click(within(cartItemContainers[0]).getByTestId('cart-item-delete-quantity-link'));
    expect(axios.delete).toHaveBeenCalledWith(
      '/api/cart-items/e43638ce-6aa0-4b85-b27f-e1d07eb678c6'
    );
    expect(loadCart).toHaveBeenCalledTimes(1);

    await user.click(within(cartItemContainers[1]).getByTestId('cart-item-delete-quantity-link'));
    expect(axios.delete).toHaveBeenCalledWith(
