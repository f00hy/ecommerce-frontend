...
describe('OrdersPage component', () => {
  ...
  it('renders order details correctly', async () => {
    ...
    expect(within(orderContainers[0]).getByTestId('order-date')).toHaveTextContent('August 12');
    expect(within(orderContainers[0]).getByTestId('order-total')).toHaveTextContent('$35.06');
    expect(within(orderContainers[0]).getByTestId('order-id')).toHaveTextContent(
      '27cba69d-4c3d-4098-b42d-ac7fa62b7664'
    );

    let orderProducts = within(orderContainers[0]).getAllByTestId('order-product-details');
    ...
    expect(within(orderContainers[1]).getByTestId('order-date')).toHaveTextContent('June 10');
    expect(within(orderContainers[1]).getByTestId('order-total')).toHaveTextContent('$41.90');
    expect(within(orderContainers[1]).getByTestId('order-id')).toHaveTextContent(
    ...
    expect(within(orderProducts[0]).getByText('Intermediate Size Basketball')).toBeInTheDocument();
  });
});
